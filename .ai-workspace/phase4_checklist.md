...
- [x] **Cold start (offline parts):** `docker compose down -v && up --build`; engine boots with the agent deps (Stagehand import OK, migrations applied); **compiled-playbook replay + require-explicit-model 422 verified over real HTTP**.
- [x] **Cold start (LIVE agent, in-container):** with a funded `ANTHROPIC_API_KEY` + `CHROME_PATH` in the engine container, `scripts/docker-agent-check.ts` drives a **full live learn→replay against the dockerized engine** — the agent learns the `fixture:3100` form INSIDE the container (Chromium-in-image launch, egress to Anthropic, key + `/v1` normalize), compiles a playbook, and it replays with *different* data and no LLM. **PASS** (2026-06-17). This closes the host-only gap: the agent path is proven in the container, not just on the host.
...
**In-Docker live verification (2026-06-17) — `scripts/docker-agent-check.ts`:**
- **PASS.** Full live learn→replay run against the DOCKERIZED engine (`localhost:8080`, agent targets `http://fixture:3100`): learns in-container → compiles `pb_...` → replays with `Z999000`/`Okonkwo` deterministically, no LLM. Proves the agent path works inside the container, not just on the host (the gap the host-only `test:live` left open).
- **Container-specific fix:** Stagehand's in-container Chromium launch needs **`CHROME_PATH`** (+ `CHROMIUM_EXECUTABLE_PATH`) pointing at the base image's binary (`/ms-playwright/chromium-1228/chrome-linux/chrome`, tied to the pinned `playwright:v1.61.0-noble` image) — auto-detection that works on the host isn't enough in-container. Added to `docker-compose.yml` `engine.environment`, along with `ANTHROPIC_API_KEY`/`ANTHROPIC_BASE_URL` (interpolated from `.env`, key stays out of the image) and `ALLOW_PRIVATE_TARGETS=true` for the fixture target.
- Re-run with: `docker compose up --build -d` then `npx tsx scripts/docker-agent-check.ts`.
